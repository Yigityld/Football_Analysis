...
class_colors = {
    'Fenerbahce': (0, 255, 0),         # Yeşil (takım A)
...
Fenerbahce = cv2.imread("..//Forma//Fenerbahce.png")
Hsv_Fenerbahce = cv2.cvtColor(Fenerbahce, cv2.COLOR_BGR2HSV)
ortalama_fenerbahce = np.array(cv2.mean(Hsv_Fenerbahce)[:3])


if ortalama_galatasaray[1] < ortalama_fenerbahce[1]:
    team_a_color = ortalama_galatasaray
else:
    team_b_color = ortalama_fenerbahce
...
def classify_player(hsv_value):
    ...
    print("gs", team_a_color)
    print("fb", team_b_color)
    return "Fenerbahce" if dist_to_a < dist_to_b else "Galatasaray"
