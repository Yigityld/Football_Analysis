...
# Video dosyasını aç
video_path = "../Videos/Konya_gs.mp4"
...
class_colors = {
    'Konyaspor': (0, 255, 0),         # Yeşil (takım A)
    'Galatasaray': (0, 140, 255),     # Turuncu-Sarı (takım B)
...
team_a_color = None
team_b_color = None
Galatasaray = cv2.imread("../Forma/Galatasaray.png")
Hsv_Galatasaray = cv2.cvtColor(Galatasaray, cv2.COLOR_BGR2HSV)
ortalama_galatasaray = np.array(cv2.mean(Hsv_Galatasaray)[:3])

Konyaspor = cv2.imread("../Forma/Konyaspor.png")
Hsv_Konyaspor = cv2.cvtColor(Konyaspor, cv2.COLOR_BGR2HSV)
ortalama_konyaspor = np.array(cv2.mean(Hsv_Konyaspor)[:3])

print("Galatasaray", ortalama_konyaspor[1])
print("Konyaspor" , ortalama_galatasaray[1])

if ortalama_galatasaray[1] < ortalama_konyaspor[1]:
    team_a_color = ortalama_galatasaray
else:
    team_b_color = ortalama_konyaspor
...
def classify_player(hsv_value):
    ...
    return "Konyaspor" if dist_to_a < dist_to_b else "Galatasaray"
